...
class VacancyControllerTest {
    ...
    @Test
    void whenPostCreateVacancyWithFileThenSameDataAndRedirectToVacanciesPage() throws Exception {
    ...
    @Test
    void whenPostCreateVacancySomeExceptionThrownThenGetErrorPageWithMessage() {
    ...
    @Test
    void whenPostUpdateVacancyWithFileThenSameDataAndRedirectToVacanciesPage() throws IOException {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    void whenPostUpdateVacancySomeExceptionThrownThenGetErrorPageWithMessage() {
        var expected = new RuntimeException("Вакансия с указанным идентификатором не найдена");
        when(vacancyService.update(any(), any())).thenThrow(expected);
        var model = new ConcurrentModel();
        var view = vacancyController.update(new Vacancy(), testFile, model);
        var actualExceptionMessage = model.getAttribute("message");
        assertThat(view).isEqualTo("errors/404");
        assertThat(actualExceptionMessage).isEqualTo(expected.getMessage());
    }
